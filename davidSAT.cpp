...
bool propagateGivesConflict ( ) {
  while ( indexOfNextLitToPropagate < modelStack.size() ) {
    ...
    for (uint i = 0; i < claus->size(); ++i) {
      ...
      int numUndefs = 0;
      int lastLitUndef = 0;
      int clausulaAct = (*claus)[i];
      for (uint k = 0; not someLitTrue and k < clauses[clausulaAct].size(); ++k){
	      int val = currentValueInModel(clauses[clausulaAct][k]);
	      if (val == TRUE) someLitTrue = true;
	      else if (val == UNDEF){
          ++numUndefs; lastLitUndef = clauses[clausulaAct][k]; 
        }
      }
